...
def mutualToroidalDistances(points, identityLarge=False):
    points = np.expand_dims(points, 0)
    pointsExtended = np.expand_dims(points, -1)
    # transposed = np.transpose(pointsExtended, perm=[0, 3, 2, 1])
    transposed = pointsExtended.transpose(0, 3, 2, 1)
    absDiff = np.abs(pointsExtended - transposed)
    torDiff = np.where(np.greater(absDiff, 1.5), 3 - absDiff, absDiff)
    # print(torDiff.shape)
    squaredDst = np.sum(np.square(torDiff), 2)
    # print(squaredDst.shape)
    # gradient of sqrt(0) is infinite => clamp
    SQRT_EPS = 10e-12
    clipped = np.maximum(squaredDst, SQRT_EPS)
    dst = np.sqrt(clipped)
    dst = dst[0]
    np.fill_diagonal(dst, 10e8)
    dst = dst.flatten()
    # print(dst.shape, dst.min(), dst.max())
...
# if we only compare it doesn't matter if it's squared
def min_dist_squared_toroidal(points, point):
    # print('points:', points.shape, points.min(), points.max())
    # print('point:', point.shape, point.min(), point.max())
    diff = points - np.array([point])
    absDiff = np.abs(diff)
    torDiff = np.where(np.greater(absDiff, 1.5), 3 - absDiff, absDiff)
    dist = np.sum(np.square(torDiff), -1)
    return np.min(dist)
...
class PoissonGenerator:
    ...
    def find_next_point(self, current_points, iterations_per_point):
        ...
        for new_point in random_points:
            ...
            # dist = min_dist_squared_toroidal(current_points, new_point)
            if dist > best_dist:
                best_dist = dist
    ...
    def find_point_set(self, num_points, num_iter, iterations_per_point, rotations, progress_notification=None):
        ...
        for i in range(num_iter):
            ...
            if rotations > 1:
                ...
                # print('points_permuted:', points_permuted.shape)
            else:
                current_set_dist = np.min(scipy.spatial.distance.pdist(points))  # only compute num_points*(num_points-1) distances
                # current_set_dist = np.min(mutualToroidalDistances(points))
                # print('points:', points.shape, points.min(), points.max())
