...
from utility.plot import plot_mc_point_np, plot_mc_disk_np, plot_img, plot_grid, plot_mc_feature_np
from utility.knn import get_knearest_neighbors

from models.gabor_torch_multiattributes import ContinuousGaborFilterBanks

from utils import normalization_torch
from models.normal_cnn_multiattributes import CNN, init_conv_weights
...
class Optimizer:
    def __init__(self, opt):
        ...
        # print(self.output_folder, self.folder_name)
        ...
        if opt.test_data == '../test_data/testset_disk':
            ...
            from dataloaders.dataloader_test_multiattributes import TestLoader
        else:
    ...
    def opt_seq(self, opt):
        ...
        for i, (x, c, r, x_l, c_l, r_l, _, x_name) in enumerate((self.test_loader_A)):
            ...
            kernel_sigma1 = (distances1.mean() / opt.kernel_sigma1).tolist()
            kernel_sigma2 = (distances1.mean() / opt.kernel_sigma2).tolist()
            print('kernel_sigma:', kernel_sigma1, kernel_sigma2)

            img_res = torch.round(9 / sorted_distances.mean()).tolist()
            ...
            plot_mc_disk_np(x.detach().cpu().numpy(), r.detach().cpu().numpy(), c.detach().cpu().numpy(), 4, self.all_test_output + '/{:}_exemplar_n{:}'.format(x_name, x.shape[0]),
                            'pdf')
            plot_mc_disk_np(x.detach().cpu().numpy(), r.detach().cpu().numpy(), c.detach().cpu().numpy(), 4, self.all_test_output + '/{:}_exemplar_n{:}'.format(x_name, x.shape[0]),
                            'png')

            pts_tar = np.concatenate([c.unsqueeze(1).detach().cpu().numpy(), x.detach().cpu().numpy(), r.detach().cpu().numpy()], 1)
            np.savetxt(self.all_test_output + '/{:}_exemplar_n{:}.txt'.format(x_name, pts_tar.shape[0]), pts_tar)
            ...
            for k in range(num_classes):
                ...
                if init_strategy == 1:
                    ...
                    init_rad[k] = tar_rad.repeat(self.upscaling_rate ** 2, 1) / self.upscaling_rate
            ...
            plot_mc_disk_np(all_init, all_init_rad, all_init_cls, 1, self.all_test_output + '/{:}_init_n{:}'.format(x_name, all_init.shape[0]), 'pdf')
            plot_mc_disk_np(all_init, all_init_rad, all_init_cls, 1, self.all_test_output + '/{:}_init_n{:}'.format(x_name, all_init.shape[0]), 'png')

            pts_all_init = np.concatenate([np.expand_dims(all_init_cls, 1), all_init, all_init_rad], 1)
            np.savetxt(self.all_test_output + '/{:}_init_n{:}.txt'.format(opt.scene_name, pts_all_init.shape[0]), pts_all_init)
            ...
            for k in range(num_classes):
                ...
                pts_k = init_pts[k]
                rad_k = init_rad[k]
                all = np.concatenate([cls_k.unsqueeze(1).detach().cpu().numpy(), pts_k, rad_k.detach().cpu().numpy()], 1)
            ...
            print('pts_txt_save:', pts_txt_save.shape)
            self.learned_filter = CNN(opt, num_attributes=pts_txt_save[:, 3:].shape[1]).to(self.device).eval()
            ...
            """ main loop: be careful of re-initialization of stuff using self.init_all """
            for k in range(num_classes):
                ...
                for j in range(len(kernel_sigmas)):
                    ...
                    slicing_exemplar = gabor_im_features.copy()[1:]
                    ...
                    for epoch in tqdm(range(epochs)):
                        ...
                        if epoch % opt.log_freq == 0 or epoch == epochs - 1:
                            ...
                            num_feature_dim = out_rad.shape[1]

                            pts_txt_save = np.concatenate([
                                np.expand_dims(out_cls.detach().cpu().numpy(), 1),
                                out.detach().cpu().numpy(),
                                # np.expand_dims(out_rad.detach().cpu().numpy(), 1),
                                out_rad.detach().cpu().numpy(),
                            ], 1)
                            np.savetxt(self.output_folder + '/' + folder_name + '/output/{:}_n{:}_scale{:}_epoch{:}.txt'.format(opt.scene_name, pts_txt_save.shape[0], j, epoch),
                                       pts_txt_save)
                    ...
                    np.savetxt(self.output_folder + '/' + folder_name + '/output/{:}_scale{:}.txt'.format(opt.scene_name, j), pts_txt_save)
                ...
                if num_classes > 1:
                    ...
                    # print('margin_space_out:', margin_space_out)
