...
class ContinuousGaborFilterBanks(torch.nn.Module):
    def __init__(self, device, res, upscaling_rate, use_toroidal=False, k=40):
        ...
        for theta in thetas:
            for lamda in [1, 2, 3, 4, 5]:
                for gamma in [1.0]:
                    for phi in [0, np.pi / 8, -np.pi / 8]:
                        params.append([theta, lamda, phi, gamma])
    ...
    def forward(self, pts, kernel_sigma, jitter=None, freq_min_max=None):
        res = self.res
        ...
        sigma = torch.from_numpy(np.array(kernel_sigma / self.upscaling_rate)).float().to(self.device).unsqueeze(0).unsqueeze(0).unsqueeze(0)  # .repeat(len(self.params), 1, 1)
        sigma_x = sigma
        sigma_y = sigma / gammas

        num_params = len(self.params)
        x_theta = x * torch.cos(thetas) + y * torch.sin(thetas)
        y_theta = -x * torch.sin(thetas) + y * torch.cos(thetas)

        spatial_term = torch.exp(-0.5 * (x ** 2 / sigma ** 2 + y ** 2 / sigma ** 2))
        real_val = [spatial_term]  # * (self.spectrumRes * self.spectrumRes + 1)
        # real_val = []
        # TODO: exclude the case when tao = 0
        freq_min = np.inf
        freq_max = -np.inf
        for tao in self.tao[1:]:
            ...
            freq_min = min(freq_min, rv.min())
            freq_max = max(freq_max, rv.max())

            real_val.append(rv)

        real_val = torch.cat(real_val, 0)

        real_val = torch.sum(real_val * 1, 1)

        out = real_val / k  # pts.shape[0]

        out = out.view(out.shape[0], res, res).float()
